...
class VAE(nn.Module):
    ...
    def encode(self, x: Variable) -> (Variable, Variable):
        ...
        logvar_z = self.softplus(logvar_z) + 1e-6
    ...
    def decode(self, z: Variable) -> (Variable, Variable):
        ...
        inv_prec = self.softplus(inv_prec) + 1e-6
    ...
    def loss_function(self, recon_x_batch, x, mu, logvar, BATCH_SIZE) -> Variable:
        ...
        GLL = 0.5 * torch.sum(torch.pow((x - mu_x),2) / log_sigma + torch.log(log_sigma), 1)
        KLD = -0.5 * torch.sum(1 + torch.log(logvar) - mu.pow(2) - logvar, 1)
        return GLL, KLD
    ...
    def grad(self, x_in, nsamples):
        ...
        grad_l = torch.zeros((nsamples, parfact, sz), device=self.device, dtype=torch.float)
        inv_prec_all = torch.zeros((nsamples,parfact, sz), device=self.device, dtype=torch.float)
        for ix, recon_x in enumerate(recon_batch):
            ...
            GL = -0.5 * torch.sum(torch.pow((img_ano[:,0,:] - mu_x),2) / inv_prec + torch.log(inv_prec), 1)
            KLD = 0.5 * torch.sum(1 + torch.log(logvar) - mu.pow(2) - logvar, 1)
            
            beta = 1
            ELBO = torch.sum(GL + beta * KLD)
...
def train(epoch, model, train_loader, patchsize, batch_size, optimizer, writer, beta, device):
    ...
    for ix, batch in enumerate(train_loader):
        ...
        #print(torch.mean(GLL).item(), torch.mean(beta * KLD).item())
        loss.backward()
